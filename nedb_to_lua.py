...
def main():
    ...
    for nedb_ship in nedb_ships.values():
        ...
        for k in ship:
            # pass
            # pprint(ship[k])
            if k in ('ja_jp', 'ja_kana', 'zh_cn'):
                print('{}: {}'.format(k, ship[k].encode('utf-8')))
            elif k == 'class':
                print('class: {} {}'.format(ship[k][0].encode('utf-8'),
                                            ship[k][1]))
            else:
                try:
                    print('{}: {}'.format(k, str(ship[k])))
                except UnicodeEncodeError:
                    print('{} {}'.format(ship['id'], k))
                    raise

    from pprint import pprint
    pprint(ship_id_kcwiki_id_table)
